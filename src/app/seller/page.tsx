...
export default function SellerDashboardPage() {
  ...
  // Mock graph data for weekly revenue
  const chartData = [
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        if (ordersRes.data && ordersRes.data.length > 0) {
          const totalRev = ordersRes.data.reduce((acc: number, order: any) => acc + (order.total_rupees || 0), 0);
          setStats({
            revenue: totalRev,
            orders: ordersRes.data.length,
            products: productsRes.data ? productsRes.data.length : 18
          });
        ...
        console.warn("Using dashboard fallback data", err);
        loadFallbackData();
      } finally {
        setLoading(false);
      }
    ...
    <div>
      ...
      <div className="dashboard-grid">
        <div className="dashboard-card">
          <h3>Total Revenue</h3>
          <p className="dashboard-card__value">₹ {stats.revenue.toLocaleString('en-IN')}</p>
        ...
        <div className="dashboard-card">
          ...
          <span className="dashboard-card__trend positive">Handcrafted catalog</span>
        </div>
      </div>
      ...
      <div style={{ marginTop: '48px', background: 'var(--pure-white)', padding: '24px', borderRadius: '8px', border: '1px solid var(--soft-gold-line)', boxShadow: '0 4px 12px rgba(43,31,24,0.03)' }}>
        <h3 style={{ marginBottom: '24px', fontSize: '16px', color: 'var(--ink-brown)' }}>Weekly Revenue Trajectory</h3>
      ...
      <div style={{ marginTop: '48px' }}>
        <h2 style={{ marginBottom: '24px' }}>Recent Atelier Activity</h2>
        <div className="data-table-container">
          <table className="data-table">
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={4} style={{ textAlign: 'center' }}>Loading orders...</td></tr>
